  ...
  class VRing {
   public:
    VRing(VirtioDriver<VirtType>& driver, uint16_t qsize, size_t idx, Nid nid)
      ...
      used_event_ =
          reinterpret_cast<std::atomic<uint16_t>*>(avail_ring_end);
      ...
      avail_event_ =
          reinterpret_cast<std::atomic<uint16_t>*>(used_ring_end);
   ...
   private:
    ...
    Avail* avail_;
    Used* used_;
    std::atomic<uint16_t>* avail_event_;
    std::atomic<uint16_t>* used_event_;
    uint16_t qsize_;
    uint16_t last_used_;
    uint16_t avail_idx_;
    uint16_t used_head_;
