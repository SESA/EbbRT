...
struct pvclock_wall_clock {
  ...
  std::atomic<uint32_t> sec;
  std::atomic<uint32_t> nsec;
};
...
struct pvclock_vcpu_time_info {
  ...
  std::atomic<uint64_t> tsc_timestamp;
  std::atomic<uint64_t> system_time;
  std::atomic<uint32_t> tsc_to_system_mul;
  std::atomic<int8_t> tsc_shift;
  std::atomic<uint8_t> flags;
  std::atomic<uint8_t> pad[2];
};
