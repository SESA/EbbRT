...
//     if (flags & kTcpAck) {
//       if (TcpSeqBetween(info.ackno, snd_una + 1, snd_nxt)) {
//         // This is an ACK for the SYN-ACK, the three-way handshake is
// complete
//         state = kEstablished;
...
//     break;
//   case kFinWait1:
//     kprintf("FinWait1: rcv_nxt: %x (%x - %x)\n", rcv_nxt, info.seqno,
//             info.seqno + info.tcplen - 1);
//     Receive(ih, th, info, std::move(buf));
//     kprintf("FinWait1: rcv_nxt: %x (%x - %x)\n", rcv_nxt, info.seqno,
//             info.seqno + info.tcplen - 1);
...
//         flags & kTcpFin) {
//       if (flags & kTcpAck && info.ackno == snd_nxt) {
//         kprintf("FIN/ACK\n");
//         // Got FIN/ACK -> Transition to TIME_WAIT state
//         SendEmptyAck();
//         Purge();
//         DisableTimers();
...
//       } else {
//         kprintf("FIN\n");
//         // Got FIN -> simultaneous close, transition to CLOSING
//         SendEmptyAck();
//         state = kClosing;
//       }
//     } else if (flags & kTcpAck && info.ackno == snd_nxt) {
//       kprintf("ACK\n");
//       // Got Ack -> Transition to FinWait2
//       state = kFinWait2;
//     }
//     break;
//   case kFinWait2:
...
//     if (TcpSeqBetween(rcv_nxt, info.seqno, info.seqno + info.tcplen - 1) &&
//         flags & kTcpFin) {
//       // Got Fin -> Transition to TIME_WAIT state
//       SendEmptyAck();
//       Purge();
//       DisableTimers();
...
//     Receive(ih, th, info, std::move(buf));
//     if (flags & kTcpAck && info.ackno == snd_nxt) {
//       // Got Ack -> Transition to TIME_WAIT state
//       Purge();
//       DisableTimers();
...
//     kprintf("TimeWait\n");
//     if (info.tcplen > 0 &&
//         !TcpSeqBetween(rcv_nxt, info.seqno, info.seqno + info.tcplen - 1)) {
//       kprintf("Empty ack\n");
//       // We received data and its not in sequence, send empty ack
//       SendEmptyAck();
//     } else if (flags & kTcpSyn) {
//       // We received a SYN in window, send a reset
//       reset = true;
...
//   int32_t remaining = rcv_wnd - (snd_nxt - snd_una);
//   return remaining >= 0 ? remaining : 0;
// }

// // Receive a segment on a connection. Returns whether or not the data (if
// there
// // was any) was in the window
// bool ebbrt::NetworkManager::TcpEntry::Receive(const Ipv4Header& ih,
//                                               const TcpHeader& th,
//                                               const TcpInfo& info,
//                                               std::unique_ptr<MutIOBuf> buf)
// {
...
//   // ACK processing
//   if (flags & kTcpAck) {
//     if (TcpSeqLT(snd_una, info.ackno)) {
...
//       // Remove all unacked segments that have been completely acked by this
// ACK
//       clear_acked_segments(unacked_segments);
//       // Its also possible to find pending segments which have been ACKed
//       // because we may have hit a retransmit timer which would cause them to
// be
...
//   if (info.tcplen > 0) {
//     // Data processing only if theres data in window and we are not in the
//     // CloseWait, Closing, LastAck, or Time-Wait state
//     if (TcpSeqBetween(rcv_nxt, info.seqno, info.seqno + info.tcplen - 1) &&
//         state < kCloseWait) {
//       buf->Advance(rcv_nxt - info.seqno);
//       // TODO(dschatz): This could overrun our local receive window, we
// should
...
//     } else if (TcpSeqLT(info.seqno + info.tcplen - 1, rcv_nxt)) {
//       // We received data which we have already acked (or this packet is from
//       // the wrong connection), send back an empty ack
//       SendEmptyAck();
//     } else {
//       // We received out of sequence data (or this packet is from the wrong
//       // connection), send back an empty ack
//       // TODO(dschatz): potentially queue this data to prevent retransmits
//       SendEmptyAck();
//     }
//   } else if (!TcpSeqBetween(info.seqno, rcv_nxt, rcv_nxt + rcv_wnd - 1)) {
//     kabort("UNIMPLEMENTED: empty ack, may need to ack back\n");
//   }

//   if (info.tcplen - ((flags & (kTcpFin | kTcpSyn)) ? 1 : 0) &&
//       state >= kCloseWait) {
//     // If there is data and we are not going to pass it to the application
//     // (because it closed the connection), we must reset to indicate that
// data
//     // was lost
//     network_manager->TcpReset(info.ackno, info.seqno + info.tcplen, ih.dst,
//                               ih.src, info.dst_port, info.src_port);
