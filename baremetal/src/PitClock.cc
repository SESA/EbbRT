...
namespace {
  ...
  double CalibrateSystemTime() {
    ...
    //   Mode 1 (One Shot) is 0x34 (from Detect CPU Speed link) but seems like could
    ...
    // Calculate speed  
    return ( ((double)total_tsc * freq) / total_tick );
  }
...
void ebbrt::clock::PitClock::ApInit() {
...
ebbrt::clock::PitClock::PitClock() {
  boot_system_time = ebbrt::rdtsc();
  ns_per_tick      = (double)1000000000 / CalibrateSystemTime();
}
...
std::chrono::nanoseconds ebbrt::clock::PitClock::TscToNano(uint64_t tsc) noexcept {
