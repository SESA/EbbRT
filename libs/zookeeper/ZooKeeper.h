...
class ZooKeeper : public ebbrt::Timer::Hook {
 public:
  enum struct Event : int { // event values declared in zookeeper/src/zk_adaptor.h
    Nil = 0,
  ...
  struct ChildWatcher : public ebbrt::ZooKeeper::Watcher {
     ...
     public:
      ...
      void WatchHandler(int type, int state, const char* path) override {
        if (type == type_)
          ...
          ebbrt::kabort("WatchEvent for non-specified type: %d (configured: %d)\n",
                        type, type_);
        }
      }
     private:
    ...
    Future<Znode> New(const std::string& path,
                      const std::string& value = std::string(), ZooKeeper::Flag  flags = ZooKeeper::Flag::Nil );
