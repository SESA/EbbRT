...
//       buf_(static_cast<uint8_t*>(buf), std::move(free_func)),
// length_(capacity),
//       capacity_(capacity) {}
...
//                             std::function<void(void*)> free_func) {
//   return std::unique_ptr<IOBuf>(new IOBuf(buf, capacity,
// std::move(free_func)));
// }

// std::unique_ptr<ebbrt::IOBuf> ebbrt::IOBuf::WrapBuffer(const void* buf,
//                                                        size_t capacity) {
//   // FIXME(dschatz): We should actually prevent the data from being written
// to
