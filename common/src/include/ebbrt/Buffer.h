...
//   static std::unique_ptr<IOBuf> Clone(const std::unique_ptr<const IOBuf>&
// buf);
...
//   /* Remove this IOBuf from its current chain and return a unique_ptr to it.
// Do
...
//       if (!p_)
//         throw std::runtime_error("DataPointer::Advance(): past end of
// buffer");
